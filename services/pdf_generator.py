...
from services.pdf_styles import create_pdf_styles
from templates.pdf_components import (
    create_cover_page,
    create_dashboard,
    create_match_pages,
...
class PDFGenerator:
    ...
    def generate(
        ...
        try:
            ...
            if 'date' not in member_data:
                member_data['date'] = datetime.now().strftime("%B %d, %Y at %I:%M %p")
            ...
            if member_id:
                ...
                filename = f"{safe_participant}_{timestamp}_JV_Report.pdf"
    ...
    def generate_to_bytes(self, member_data: Dict, member_id: Optional[str] = None) -> bytes:
        """
        Generate PDF and return as bytes (useful for Streamlit downloads)
